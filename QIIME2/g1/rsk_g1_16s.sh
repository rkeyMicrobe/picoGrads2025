...
conda create -n qiime2-community-2022.11 \
  -c conda-forge -c bioconda \
  -c https://packages.qiime2.org/qiime2/2022.11/passed/community/ \
  qiime2-community
...
qiime tools import \
  	...
  	--input-path "manifest_g1_16s.txt" \
  	--output-path "./qza/g1_16s_rawSeqs.qza" \
  	--input-format PairedEndFastqManifestPhred33V2

qiime demux summarize \
	--i-data "./qza/g1_16s_rawSeqs.qza" \
	--o-visualization "./qzv/g1_16s_rawSeqs.qzv"  
	# NOTE: .qzv are visual files; .qza are artifact files
	# View .qzv files at https://view.qiime2.org/ on a Chrome Browser (personally, it didn't work on my computer's FireFox)
	# Drag n drop 
...
qiime cutadapt trim-paired \
	--i-demultiplexed-sequences "./qza/g1_16s_rawSeqs.qza"  --p-cores 20 \
	...
	--p-discard-untrimmed  --p-no-indels --p-error-rate 0.2  --o-trimmed-sequences "./qza/g1_16s_trimmed.qza"

qiime demux summarize \
	--i-data "./qza/g1_16s_trimmed.qza" \
	--o-visualization "./qzv/g1_16s_trimmed.qzv"
...
chmod 755 *.py 
wget https://github.com/jfq3/Miscellaneous-scripts/raw/master/test_figaro.sh # once you do all this, just copy/paste to other directories if you need it there.
...
cp ./prok/V4_515F_New_V4_806R_New-63683_CACGAAGAGC.R1.fastq.gz ${dir}
cp ./prok/V4_515F_New_V4_806R_New-63683_CACGAAGAGC.R2.fastq.gz ${dir}

gzip -d ${dir}/*.fastq.gz

mv ${dir}/V4_515F_New_V4_806R_New-63683_CACGAAGAGC.R1.fastq ${dir}/sam1_16s_R1.fastq
mv ${dir}/V4_515F_New_V4_806R_New-63683_CACGAAGAGC.R2.fastq ${dir}/sam1_16s_R2.fastq
...
qiime dada2 denoise-paired \
	--i-demultiplexed-seqs "./qza/g1_16s_trimmed.qza" \
	--p-trunc-len-f 87 --p-max-ee-f 2 \
	--p-trunc-len-r 206 --p-max-ee-r 2 \
	--p-n-threads 12 \
	--o-representative-sequences "./qza/g1_16s_reprSeqs.qza" \
	--o-table "./qza/g1_16s_reprSeqs_countTable.qza" \
	--o-denoising-stats "./qza/g1_16s_denoiseStats.qza"

# Step 5a. Obtain your count matrix and denoising stats for visuals
qiime feature-table tabulate-seqs \
	--i-data "./qza/g1_16s_reprSeqs.qza" \
	--o-visualization "./qzv/g1_16s_reprSeqs.qzv"

qiime metadata tabulate \
	--m-input-file "./qza/g1_16s_denoiseStats.qza" \
	--o-visualization "./qzv/g1_16s_denoiseStats.qzv"
...
dir="g1/qza" # Change to fit your pathway structure

qiime feature-classifier classify-sklearn \
  --p-n-jobs 8 \
  --i-classifier "./${dir}/silvaClassifier_nr99_515F.806R.qza" \
  --i-reads "./${dir}/g1_16s_reprSeqs.qza" \
  --o-classification "./${dir}/g1_16s_reprSeqs_taxAligned.qza"
...
qiime metadata tabulate \
  --m-input-file "./qza/g1_16s_reprSeqs_taxAligned.qza" \
  --o-visualization "./qzv/g1_16s_reprSeqs_taxAligned.qzv"
...
# A raxml tree takes a lot of time, I recommend jumping to hipergator. The more cores, the better. For perspective: 18CPUs on a 1.3MB input took 1hr, 10min to complete. 
qiime phylogeny align-to-tree-mafft-raxml \
  --p-n-threads 18 \
  --i-sequences "./qza/g1_16s_reprSeqs.qza" \
  --o-alignment "./qza/g1_16s_reprSeqs_align.qza" \
  --o-masked-alignment "./qza/g1_16s_reprSeqs_alignMask.qza" \
  --o-tree "./trees/g1_16s_treeUnRoot.qza" \
  --o-rooted-tree "./trees/g1_16s_treeRoot.qza"

# Step 9 (optional). Export qza files to files for analysis in the R-environment.

# Step 9a. Make a tree file. 
qiime tools export \
  --input-path "./trees/g1_16s_treeUnRoot.qza" \
  --output-path "./trees"
mv ./trees/tree.nwk ./trees/g1_16s_raxmlUnroot.nwk

qiime tools export \
  --input-path "./trees/g1_16s_treeRoot.qza" \
  --output-path "./trees"
mv ./trees/tree.nwk ./trees/g1_16s_raxmlRoot.nwk

## Step 9a. Generate a biom for ASV counts, then convert it to a tsv file
qiime tools export \
  --input-path "./qza/g1_16s_reprSeqs_countTable.qza" \
  --output-path "./phyloseq"

biom convert \
  -i "./phyloseq/feature-table.biom" \
  -o "./phyloseq/g1_16s_countTable.tsv" \
  --to-tsv

sed -i '1d' ./phyloseq/g1_16s_countTable.tsv
sed -i 's/#OTU ID//' ./phyloseq/g1_16s_countTable.tsv

## Step 9b. Export representative sequences. Convert to txt file
qiime tools export \
  --input-path "./qza/g1_16s_reprSeqs.qza"  \
  --output-path "./phyloseq"
awk 'BEGIN{RS=">"}{print "#"$1"\t"$2;}' ./phyloseq/dna-sequences.fasta | tail -n+2 > ./phyloseq/g1_16s_dnaSeqs.txt
awk 'BEGIN{RS="#"}{print "#"$1"\t"$2;}' g1_18s_dnaSeqs.txt | tail -n+2 > test.csv

## Step 9c. Export Taxonomy
qiime tools export \
  --input-path "./qza/g1_16s_reprSeqs_taxAligned.qza"  \
  --output-path "./phyloseq"
mv ./phyloseq/taxonomy.tsv ./phyloseq/g1_16s_taxonomy.tsv
